...
class MotorThread(QThread):
    ...
    def camera_move(self):
        ...
        sleep(1)
        self.lib.send_motor(self.fd_motor,Addr_motor,0x0004,0x012C)  #设置转速为300r/min
        sleep(1)
    ...
    def board_out(self):
        ...
        sleep(1)
        self.lib.send_motor(self.fd_motor,Addr_board,0x0006,0x002F)  #设置圈数为 50 圈（1A）
        sleep(1)
    ...
    def board_back(self):
        ...
        self.lib.send_motor(self.fd_motor,Addr_board,0x0004,0x00C8)  #设置转速为 200 r/min
        sleep(1)
        self.lib.send_motor(self.fd_motor,Addr_board,0x0006,0x0032)  #设置圈数为 50 圈（22）
        sleep(1)
        self.lib.send_motor(self.fd_motor,Addr_board,0x0002,0x0001)  #设置电机开始运行 
        #sleep(12)
...
class YOLOThread(QThread):
    ...
    distance_signal = pyqtSignal(int,float)  #新增信号：爬电距离
    detection_color_signal = pyqtSignal(int)
    ...
    def run(self):
        ...
        self.state_updated.emit(1,"1#绝缘子测量中")
        self.perform_detection()
        self.state_updated.emit(1,"1#绝缘子已测量")
        self.detection_color_signal.emit(1)
        ...
        self.state_updated.emit(2,"2#绝缘子测量中")
        self.perform_detection()
        self.state_updated.emit(2,"2#绝缘子已测量")
        self.detection_color_signal.emit(2)
        ...
        self.state_updated.emit(3,"3#绝缘子测量中")
        self.perform_detection()
        self.state_updated.emit(3,"3#绝缘子已经测量")
        self.detection_color_signal.emit(3)

        #摄像头复位
        self.status_updated.emit("摄像头复位中...")
        self.motor_thread.set_task("camera_back")
        self.status_updated.emit("11111111111111111111111111111")
        #sleep(5)  #这里要计算电机运动的时长，等待复位
    ...
    def perform_detection(self):
        ...
        for i in range(10):
            distance, processed_image_1 = YOLO_run(self.frames,
        self.cap_left,
        self.cap_right,
        self.map_lx,
        self.map_ly,
        ...
        self.pool)
            if processed_image_1 is not None:
                processed_image_1 = cv2.flip(np.rot90(processed_image_1), 0)
                processed_image = processed_image_1.copy()
        # (h,w) = processed_image.shape[:2]
        # processed_image = np.rot90(processed_image)
        # center = ( 0, 0 )
        # M = cv2.getRotationMatrix2D(center, 90, 1)
        # processed_image = cv2.warpAffine(processed_image, M, (h, w))
        # 获取检测结果图片（示例：假设从frames中获取处理后的图像）
        cv2.imwrite('ahjsdk.jpg', processed_image)
        processed_image = QImage(processed_image.data,processed_image.shape[1],processed_image.shape[0],processed_image.shape[1],QImage.Format.Format_Indexed8)
        
        self.detection_image_signal.emit(self.detection_count,processed_image)
        self.distance_signal.emit(self.detection_count,distance)
        if distance:
            self.status_updated.emit("检测完成！")
        else:
            self.status_updated.emit("未检测到！")
...
"")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.pushButton_3.setText(_translate("MainWindow", "卸载绝缘子"))
...
class MainWindow(QMainWindow,Ui_MainWindow):
    def __init__(self):
        ...
        self.status_labels = {
            1: self.label_2,   # 对应1#绝缘子待检测标签
            2: self.label_26,  # 对应2#绝缘子待检测标签
            3: self.label_34   # 对应3#绝缘子待检测标签
        }
        ...
        self.yolo_thread.distance_signal.connect(self.update_distance_table)
        self.yolo_thread.state_updated.connect(self.update_state)  #更新测量状态
        self.yolo_thread.detection_color_signal.connect(self.update_label_solor)
        self.yolo_thread.board_signal.connect(self.switch_board)
        # 连接双目摄像头帧  更新信号:
        self.camera_thread.frame_updated.connect(self.update_camera_display)
        
    def update_label_solor(self,detection_count: int):
        """更新对应标签的背景颜色为绿色"""
        target_label = self.status_labels.get(detection_count)
        if target_label:
            # 设置绿色背景（RGB值：170, 255, 127）
            target_label.setStyleSheet("background-color: rgb(170, 255, 127); border-radius: 5px;")
    def update_detection_display(self, count: int, image: QImage):
        ...
        if count in label_map:
            ...
            # 确保label的scaledContents为False，我们手动控制缩放
            ...
            # 计算label的可用大小（减去边距）
            margins = target_label.contentsMargins()
            available_width = target_label.width() - margins.left() - margins.right()
            available_height = target_label.height() - margins.top() - margins.bottom()
    ...
    def update_distance_table(self, detection_count: int, distance: float):
        """更新检测数据并设置表格列居中"""
        row_map = {1: 0, 2: 1, 3: 2}
        row = row_map.get(detection_count, -1)
        if row == -1:
            return
        
        # 计算爬电比距（保留1位小数）
        specific_distance = round(distance / 12, 1)
        
        # 第1列：爬电距离(mm)
        distance_item = QtWidgets.QTableWidgetItem(f"{distance:.2f} mm")
        distance_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)  # 居中
        self.tableWidget.setItem(row, 1, distance_item)
        
        # 第2列：爬电比距(mm/kV) - 修正变量名
        specific_item = QtWidgets.QTableWidgetItem(f"{specific_distance:.1f} mm/kV")
        specific_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)  # 居中
        self.tableWidget.setItem(row, 2, specific_item)
        
        # 第3列：等级判定（示例逻辑）
        level = "合格"  # 根据实际需求修改判定逻辑
        level_item = QtWidgets.QTableWidgetItem(level)
        level_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
        self.tableWidget.setItem(row, 3, level_item)
    ...
    def update_camera_display(self, camera_id, q_img):
            ...
            if camera_id in self.camera_labels:
                ...
                available_width = label.width() - margins.left() - margins.right()
                available_height = label.height() - margins.top() - margins.bottom()
                
                # 计算保持宽高比的最佳尺寸
                img_width = q_img.width()
                img_height = q_img.height()
    ...
    def start_detection(self):
        ...
        self.label_33.clear()
        #恢复未测量状态：
        self.label_2.setText("1#绝缘子待检测") 
        self.label_26.setText("2#绝缘子待检测")
        self.label_34.setText("3#绝缘子待检测")
    ...
    def update_state(self,int,message):
        """更新状态信息"""
        if int is 1:
            self.label_2.setText(message)
        if int is 2:
            self.label_26.setText(message)
        if int is 3:
            self.label_34.setText(message)
